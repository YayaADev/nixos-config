...
in
{
  ...
  # Create qbittorrent user and group for volume permissions
  users.users.${qbtUser} = {
    isSystemUser = true;
    group = qbtGroup;
    home = "/var/lib/${qbtUser}";
    createHome = true;
    uid = 1000; # Set a consistent UID for container mapping
  };

  users.groups.${qbtGroup} = {
    gid = 1000; # Set a consistent GID for container mapping
  };

  systemd.tmpfiles.rules = serviceHelpers.createServiceDirectories "qbittorrent" serviceConfig ++ [
  ...
  virtualisation.oci-containers.containers = {
    gluetun = {
      ...
      ports = [
        ...
        "8000:8000/tcp" # Gluetun control API
      ];
    };
    ...
    qbittorrent-nox = {
      ...
      extraOptions = [ 
        ...
        # Remove --user flag to let container handle user mapping
      ];
      ...
      environment = {
        QBT_LEGAL_NOTICE = "confirm"; # Updated environment variable
        QBT_VERSION = "latest";
        QBT_WEBUI_PORT = toString serviceConfig.port;
        PUID = "1000"; # Set user ID
        PGID = "1000"; # Set group ID
        TZ = config.time.timeZone;
      };
    };
  };

  # Health check service for both containers
  systemd.services.qbittorrent-health-check = {
    description = "qBittorrent-nox and Gluetun health check";
    after = [ "podman-gluetun.service" "podman-qbittorrent-nox.service" ];
    wants = [ "podman-gluetun.service" "podman-qbittorrent-nox.service" ];
    serviceConfig = { 
      Type = "oneshot"; 
      User = "nobody"; 
      Group = "nogroup"; 
  ...
  systemd.timers.qbittorrent-health-check = {
